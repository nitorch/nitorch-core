...
import torch
from .tensors import as_tensor, movedim, fast_movedim, to_max_backend
from .broadcast import expanded_shape
...
def sym_matvec(mat, vec):
    ...
    mat, vec = to_max_backend(mat, vec)
    ...
    mat = fast_movedim(mat, -1, 0)
    vec = fast_movedim(vec, -1, 0)
...
def sym_solve(mat, vec, eps=None):
    ...
    shape = expanded_shape(mat.shape[1:], vec.shape[1:])
    ...
    if nb_prm == 1:
        ...
        vec = fast_movedim(vec, 0, -1)
        mat = fast_movedim(mat, 0, -1)
        mat = sym_to_full(mat)
        return _solve_lu(mat, vec.unsqueeze(-1)).squeeze(-1)

    return fast_movedim(res, 0, -1)
