...
from nitorch_core.constants import inf, ninf
from nitorch_core.version import torch_version
from nitorch_core.tensors import slice_tensor, backend, ind2sub, movedim
...
def _reduce_index(fn, input, dim=None, keepdim=False, omitnan=False,
    ...
    input = movedim(input, dim, -1)    # move reduced dim to the end
...
def _add_class(x, bg, dim, index):
    ...
    if isinstance(bg, (int, float)):
        bg = torch.as_tensor(bg, **backend(x))
