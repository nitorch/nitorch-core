...
from . import py, constants
from .tensors import as_tensor, backend
...
def fill_sym_(a, upper=True):
    ...
    """
    mask = mask_tri(a.shape[-1], diagonal=1, upper=upper, **backend(a))
...
def matvec_sym(a, x, upper=True):
    ...
    def matvec_sym_upper(a, x):
        n = a.shape[-1]
        if n <= 4:
            shape = expanded_shape(a.shape[:-2], x.shape[:-1])
            shape = [*shape, n]
            y = x.new_zeros(shape)
            for i in range(n):
                for j in range(n):
...
def outer_sym(u, v):
    ...
    if n > 4:
        ...
        return out
    shape = expanded_shape(u.shape[:-1], v.shape[:-1])
    shape = [*shape, n, n]
...
def hessenberg_sym_lower_(a, compute_u=False, fill=False):
    ...
    for k in range(n-2):
        ...
        vk *= 2
        uvk = outer_sym(uk, vk)
        msk = mask_tri(n-k-1, upper=False, **backend(a))
        ak[..., msk] -= uvk[..., msk]
        # Set k-th column to [alpha; zeros]
        a[..., k+1, k] = alpha
        a[..., k+2:, k] = 0
